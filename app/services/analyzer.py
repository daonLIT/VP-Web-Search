...
class DataAnalyzer:
    ...
    def _perform_analysis(
        ...
        prompt = f"""
당신은 보이스피싱 시뮬레이션 시스템의 데이터 분석 전문가입니다.
대화 내용을 깊이 분석하여 피해자의 취약점을 파악하고, 이를 공략할 수 있는 검색 쿼리를 생성하세요.
...
다음 JSON 형식으로 출력하세요:
{{
    "summary": "대화의 핵심 내용을 2-3문장으로 요약 - 피해자의 반응, 심리 상태, 취약한 부분 중심",
    "key_points": ["대화에서 발견된 핵심 포인트 1", "핵심 포인트 2", ...],
    "extracted_profile": {{
        "age_group": "연령대 (예: 30대, 60대 이상) 또는 null",
        "occupation": "직업 또는 null",
        "gender": "성별 또는 null",
        "characteristics": ["대화에서 파악된 특징1", "특징2"]
    }},
    "detected_scenario": "감지된 시나리오 유형 (예: 검경 사칭, 대출 사기) 또는 null",
    "vulnerability_areas": [
        "대화에서 발견된 취약 영역 1 (예: 권위에 대한 복종)",
        "취약 영역 2 (예: 금융 불안)",
        ...
    ],
    "search_queries": [
...
search_queries 생성 규칙 (중요 - 7:3 비율):
[대화 기반 쿼리 7개] - 대화 내용에서 파악된 정보 활용:
- 피해자 특성: "[연령대] [직업] 보이스피싱 취약점"
- 시나리오 기반: "[시나리오 유형] 사칭 최신 수법"
- 심리적 취약점: "[대화에서 발견된 심리 상태] 악용 수법"
- 대화 패턴: "[피해자 반응 유형] 공략 화법"
- 상황 맥락: 대화에서 언급된 구체적 상황 관련 검색

[4차 산업 기술 쿼리 3개] - 첨단 기술 활용 수법:
- "AI 딥페이크 보이스피싱", "QR코드 피싱", "악성앱 사기" 등
